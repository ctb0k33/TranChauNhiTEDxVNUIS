class SponsorshipLevelsController < ApplicationController
  before_action :set_sponsorship_level, only: %i[ show edit update destroy ]

  # GET /sponsorship_levels or /sponsorship_levels.json
  def index
    @sponsorship_levels = SponsorshipLevel.all
  end

  # GET /sponsorship_levels/1 or /sponsorship_levels/1.json
  def show
  end
  ...
  # POST /sponsorship_levels or /sponsorship_levels.json
  def create
    @sponsorship_level = SponsorshipLevel.new(sponsorship_level_params)

    respond_to do |format|
      if @sponsorship_level.save
        format.html { redirect_to sponsorship_level_url(@sponsorship_level), notice: "Sponsorship level was successfully created." }
  ...
  # PATCH/PUT /sponsorship_levels/1 or /sponsorship_levels/1.json
  def update
    respond_to do |format|
      if @sponsorship_level.update(sponsorship_level_params)
        format.html { redirect_to sponsorship_level_url(@sponsorship_level), notice: "Sponsorship level was successfully updated." }
  ...
  # DELETE /sponsorship_levels/1 or /sponsorship_levels/1.json
  def destroy
  ...
  private
    ...
    def sponsorship_level_params
      params.require(:sponsorship_level).permit(:Level_ID, :Sponsorship_Level, :Value, :Description)
    end
end
